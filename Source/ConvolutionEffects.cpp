...
void ConvolutionEffect::prepare(int buffsize)
{
    convEngine.prepare(buffsize);
}

void ConvolutionEffect::loadFileAsIR(juce::File newFile)
{
...
void ConvolutionEffect::processBlock(juce::AudioBuffer<float> buffer)
{
    
    auto span = std::span<float>(buffer.getWritePointer(0), buffer.getNumSamples());
    convEngine.processBlock(span);
...
void ConvolutionEffect::normalizeFile()
{
